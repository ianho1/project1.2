...
def search_directors_by_zip():
    ...
    query = """ 
        SELECT DISTINCT p.name AS director_name, m.name AS series_name
        FROM People p
        JOIN Role r ON p.id = r.pid
        JOIN MotionPicture m ON r.mpid = m.id
        JOIN Series s ON m.id = s.mpid
        JOIN Location l ON m.id = l.mpid
...
def search_awards():
    ...
    query = """ 
        SELECT p.name AS person_name, m.name AS movie_name, a.award_year, COUNT(*) AS award_count
        FROM Award a 
        JOIN People p ON a.pid = p.id
        JOIN MotionPicture m ON a.mpid = m.id
        GROUP BY a.mpid, a.award_year, a.pid
        HAVING COUNT(*) > %s 
    """
...
def search_producers():
    ...
    query = """ 
        ...
        LEFT JOIN Series s ON mp.id = s.mpid 
...
def search_multiple_roles():
    ...
    query = """ 
        SELECT p.name, mp.name AS motion_picture_name, COUNT(r.role_name) AS role_count
        FROM People p
        JOIN Role r ON p.id = r.pid
        JOIN MotionPicture mp ON r.mpid = mp.id
        WHERE mp.rating > %s
...
def top_thriller_movies_boston():
    ...
    query = """
        SELECT DISTINCT m.name, m.rating
        FROM MotionPicture m
        JOIN Genre g ON m.id = g.mpid
        JOIN Location l ON m.id = l.mpid
...
def search_movies_by_likes():
    ...
    query = """ 
        SELECT mp.name, COUNT(l.mpid) AS like_count
        FROM MotionPicture mp
        JOIN Likes l ON mp.id = l.mpid
        JOIN Users u ON l.uemail = u.email
        WHERE u.age < %s
        GROUP BY mp.name
        HAVING COUNT(l.mpid) > %s
...
def actors_marvel_warner():
    ...
    query = """ 
        SELECT DISTINCT p.name AS actor_name, mp.name AS movie_name
        FROM People p
        JOIN Role r ON p.id = r.pid
        JOIN MotionPicture mp ON r.mpid = mp.id
...
def top_5_movies_people_roles():
    ...
    query = """ 
        SELECT mp.name AS movie_name, COUNT(DISTINCT r.pid) AS people_count, COUNT(DISTINCT r.role_name) AS role_count
        FROM MotionPicture mp
...
def actors_with_common_birthday():
    ...
    query = """ 
        SELECT p1.name AS actor_1, p2.name AS actor_2, p1.dob AS common_birthday
        FROM People p1
        JOIN People p2 ON p1.dob = p2.dob
        WHERE p1.id < p2.id
        ORDER BY common_birthday;
    """
